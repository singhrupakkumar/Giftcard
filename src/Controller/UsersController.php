...
class UsersController extends AppController { 
    ...
    public function beforeFilter(Event $event) {
        ...
        $this->Auth->allow(['add', 'login', 'forgot', 'reset', 'contact',
            ...
            'invitecode','paymenthistory','emailverify','myaccount','card','myplan']);  
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $post['role'] = 'user';  
            $year = $post['dob']['year'];
            $month = $post['dob']['month'];
            $day = $post['dob']['day'];


            $post['dob'] = $year."-".$month."-".$day; 
            ...
            if ($new_user) {
                ...
                if (isset($user)) { 
                 ...
                 $send = $email->from(['rupak@avainfotech.com' => 'Video']) 
                    ...
                    $this->Flash->success(__('Registration done successfully. Thanks for joining with us.'));   
...
//    
       ...
       $userplan = $this->Usersplans->find('all', [
            'contain'=>['Users','Plans'],
            'conditions'=>['AND'=>['Usersplans.payment_status'=>1,'Usersplans.user_id'=>$this->Auth->user('id')]]
        ]);

        $userplan = $userplan->first();

         }else{
             $this->Flash->error(__('Please login first for see plan'));  
            ...
            if ($new_user) {
                ...
                if (isset($user)) { 
                 ...
                 $send = $email->from(['rupak@avainfotech.com' => 'Video']) 
                    ...
                    $response['msg'] = "<div class='alert alert-success'><strong>Registration done successfully. We have sent a verification mail to your registered email address, Please verify your account. Please be patient, as it may take some time to reach your inbox.</strong></div>";
                ...
                $response['isSucess'] = "false";
                $response['msg'] = "<div class='alert alert-danger'><strong>The user could not be saved. Please, try again.</strong></div>";
        ...
        //                  $email = new Email('default');
        //                  $email->from(['rupak@avainfotech.com' => 'Video'])  
        //                  	->emailFormat('html')
        ...
        if (isset($this->request->data['action']) && $this->request->data['action'] == "fblogin") {
             $imgurl ='https://graph.facebook.com/'.$this->request->data['myid']['id'].'/picture?width=320&height=320' ;
            ...
            if (!empty($results)) {
              ...
              // $this->request->data['password'] = 'zxswedcxswzrrr';  
               // $user1 = $this->Auth->identify();  
                if ($results) {
                    $this->Users->updateAll(array('fb_id' => $this->request->data['myid']['id'],'image'=>$imgurl), array('id' => $results['id']));
                    $this->Auth->setUser($results);
                    $response['isSuccess'] = 'true';
                    $response['msg'] = '<div class="alert alert-success"><strong>Logged in successfully.</strong></div>';
                } else {
                    $response['isSuccess'] = 'false';
                    $response['msg'] = '<div class="alert alert-danger"><strong>Error In Signing In. Please Try Again.</strong></div>';
       
                }
            } else {
                $post = array();
                $post['fb_id'] = $this->request->data['myid']['id'];
                $post['fname'] = $this->request->data['myid']['first_name'];
                $post['lname'] = $this->request->data['myid']['last_name'];
    ...
    public function gplogin() {
        ...
        if (isset($this->request->data['action']) && $this->request->data['action'] == "gplogin") {
            ...
            if (!empty($results)) {
               // $user1 = $this->Auth->identify();
                if ($results) {  
                    $this->Users->updateAll(array('google_id' => $this->request->data['id'],'image'=>$this->request->data['image']), array('id' => $results['id']));
                    $this->Auth->setUser($results);  
                    $response['isSuccess'] = 'true';  
                    $response['msg'] = '<div class="alert alert-success"><strong>Logged in successfully.</strong></div>';
                } else {
                ...
                $post['fname'] = $this->request->data['first_name'];
                $post['lname'] = $this->request->data['last_name'];
    ...
    public function forgot() {
        ...
        if ($this->request->is('post')) {
            ...
            $this->Flash->error(__('Please enter your register email address'));
