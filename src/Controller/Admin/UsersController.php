...

{
	...
	public function beforeFilter(Event $event) {
        ...
        $this->Auth->allow(['logout','uservoicelist','paymenthistory']);
    ...
    public function uservoicelist($user_id = null){

        $this->loadModel('Usertones');

       $userplan = $this->Usertones->find('all', [
            'contain'=>['Users','Tones'],
            'conditions'=>['Usertones.user_id'=>$user_id]
        ]);
    ...
    public function planview($id = null) {
    	...
    	if(!$id) {
    		  return $this->redirect(['controller' => 'users', 'action' => 'customerplan']);   
    		  $this->Flash->error(__('Invalid Plans, try again')); 
    	}
       ...
       $userplan = $this->Usersplans->find('all', [
            ...
            'contain'=>['Users','Plans'],
            'conditions'=>['Usersplans.payment_status'=>1,'Usersplans.id'=>$id]  
        ]);
	...
	public function paymenthistory(){
	  ...
	  $payment = $this->Payments->find('all',['conditions'=>['Payments.status'=>1],'contain'=>['Users','Videos'],'order'=>['Payments.id'=>'desc'],'group'=>['Payments.user_id']]);
    ...
    public function paymentdetails($user_id = null){

      $this->loadModel('Payments');

      $payment = $this->Payments->find('all',['conditions'=>['Payments.status'=>1,'Payments.user_id'=>$user_id ],'contain'=>['Users','Videos'],'order'=>['Payments.id'=>'desc']]);
